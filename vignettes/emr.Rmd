...
conversion function : `convertIcdDxDecimaltoShort and convertIcdDxShortToDecimal`
...
`groupIcdDxToCCS` can get CCS categories or description for ICD-9 and ICD-10 codes on diagnoses.
...
#### CCS multi-level
`groupIcdDxToCCSLvl` can get CCS multiple levels or description for ICD-9 and ICD-10 codes on diagnoses.
...
### Phecode
`groupIcdDxToPhecode` allows  for conversion and aggregate ICD9 codes into PheWAS codes (Phecode).
...
```
 ...
 1) `groupIcdDxToCustomGrep`: Use method of exhaustion to find out the matched ICD.
```{r}
# groupingTable 
...
```
 2) `groupIcdDxToCustomGrepList`: Use a ICD pattern to find out the fit ICD.
```{r}
# grepTable
