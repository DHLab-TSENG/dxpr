...
# c("0001", "0002", "0003", "0009", "0010", "0011")
# ICD-9-PCS_Decimal
head(ICD9PrwithTwoFormat$Decimal)
# c("00.01", "00.02", "00.03", "00.09", "00.10", "00.11")
```
ICD-10-PCS只有short的格式
```r
# ICD-10-PCS_Short
head(prICD10$ICD)
# c("0016071", "0016072", "0016073", "0016074", "0016075", "0016076")
```
### 範例資料
...
疾病分組時，為能區別處置編碼版本ICD-9/ICD-10，本套件依據**診斷日期**區分處置編碼的版本，其時間切割點可依使用者需求設定。  
...
head(decimal$ICD)
# c("56.81", "97.74", "44.99", "07.59", "02.05", "88.12")
```
...
short <- IcdPrDecimalToShort(PrDataFile = samplePrFile,
                             ...
                             icd10usingDate = "2015/10/01")
head(short$ICD)
# c("5681", "9774", "4499", "0759", "0205", "8812")
```
**warning message** (詳細流程請參考[診斷前處理與整合方法](https://dhlab-cgu.github.io/emr/articles/ChineseVersion_Diagnosis.html#i--code-standarization))

醫療數據的處置編碼有可能誤植導致處置編碼錯誤，而錯誤的處置編碼將影響後續臨床疾病分組錯誤或是無法分組的情形。  
為方便使用者修改錯誤的處置編碼，emr套件會提供warning message提醒使用者些錯誤的處置編碼。  
