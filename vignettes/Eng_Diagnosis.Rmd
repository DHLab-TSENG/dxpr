...
dxpr package uses ICD-CM codes as diagnosis standard. There are two formats of ICD-9 and ICD-10 diagnostic codes, decimal (with a decimal point separating the code) and short format, respectively. So two tables of ICD-9-CM and ICD-10-CM are generated to deal with different user needs: \code{ICD9DxwithTwoFormat} and \code{ICD10DxwithTwoFormat}.
...
There are two error type：**wrong format** and **wrong version**. The former one means the ICD codes does not exist (maybe because ICD is wrongly coded or with a wrong place of decimal point). And the latter one means the version is wrong (still use ICD 9 after `icd10usingDate`, etc.).
...
And there are 9 ICD codes labeled as "wrong format", and 7 ICD labeled as "wrong version". Users can check data after receiving the warning message.
...
The original row order of the data is remained in groupedDT, and only one extra column `GroupType` is added. 
...
Multi-level CCS in ICD-9-CM has four levels, and ICD-10-CM has two levels.
```{r, message = FALSE, warning = FALSE}
