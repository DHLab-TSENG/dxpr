...
本套件將CMS (Centers for Medicare & Medicaid Services)所提供的ICD編碼視為ICD編碼的標準依據，並依照WHO訂定的ICD編碼規則，分別產生 ICD-9-CM and ICD-10-CM 的兩種格式的表格: `ICD9DxwithTwoFormat 和 ICD10DxwithTwoFormat`
...
進行後續標準化分組功能之前，將診斷紀錄轉換為有臨床意義的疾病分組之前，依據CCS（Clinical Classifications Software）、共病症（Comorbidity）及PheWAS編碼的分組表中ICD診斷碼格式如**表一**，將醫療大數據中的ICD診斷編碼進行統一編碼格式的轉換，以便後續ICD編碼標準化分群。
...
為在疾病分組時區別診斷編碼ICD-9-CM與ICD-10-CM，本套件依據診斷日期區分ICD的版本，其時間切割點可依使用者需求設定。

如使用者分析台灣的醫療資料，可將日期設定為2016年1月1日；

如使用者分析的是美國的醫療資料，可將日期設定為2015年10月1日。

以下示範皆以`sampleDxFile`作為範例, 區分ICD-9/ICD-10-CM code的日期為: 2015年10月1日
...
以下為判斷ICD編碼錯誤之流程圖，以short 轉為 decimal 作為範例
...
```r
sample <- IcdDxShortToDecimal(sampleDxFile,ICD,Date,"2015/10/01")
...
```
ICD編碼錯誤: 共9種ICD (括號內為該ICD出現次數) 

c("A0.11 (20)", "E114 (8)", "Z9.90 (6)", "F42 (6)", "001 (5)", "75.52 (4)", "755.2 (3)", "123.45 (3)", "7552 (2)")

ICD版本錯誤: 共7種ICD (括號內為該ICD出現次數) 
...
1) **groupedDT**: 呈現方式如**表二**，將ICD診斷碼依國際標準轉換為有臨床意義的疾病分組, 可用來計算疾病世代 (condition era)
...
2) **summarised_groupedDT**: 呈現方式如**表三**，將同一個ID的病患其相同標準化分組的多筆資料會統整成一筆資料的長表，取診斷資料中最早診斷的時間為first case date，診斷資料中最後記錄的時間為end case date，並計算其週期（period）內相差的天數，以及週期內共有幾筆診斷紀錄（count）。 另可將其資料轉為適合進行統計分析之資料型態的寬表。長表資料型態如下
...
Charlson database共病症測量指標的是基於Quan的Charlson Comorbidity Index版本, 使 ICD code 依其相關的共病症做分類
...
醫學研究常需要觀察事件發生前後的改變，因此需要一個指標時間（index date）來記錄事件的發生。TimeTag欄位可顯示此紀錄為事件前或是事件後（before and after），使用者可依此欄位選擇事件發生前／後的資料。

另外本套見也提供週期（window）計算功能，週期長度和指標日期由使用者自行定義。
...
## IV. Visualization
進行第一階段 - 診斷編碼格式統一時,會將資料中的錯誤編碼訊息列表並顯示其數量

`plot_errorICD`可將錯誤編碼訊息以圖表方式呈現, 使用者可選擇**單看**格式錯誤 (Wrong format)或是版本分類錯誤 (Wrong version)或顯示**全部**的錯誤編碼種類
...
...等共19章節，`plot_errorICD`依照編碼開頭分類，大致分為11組: 0, 1, 2,..., 9, V and E

使用者可選擇顯示前幾名的錯誤編碼，例如使用者設定顯示前10名，將顯示前十名的柏拉圖
（pareto plot）及前十名錯誤編碼的列表。預設為顯示前10名的錯誤編碼

藉由柏拉圖圖表，使用者可得知錯誤的種類及主要的錯誤編碼供使用者做為參考
...
plot_errorICD(error$Error, ICDVersion = all,
                           ...
                           Others = FALSE)
```
![pareto plot of error ICD](https://dhlab-cgu.github.io/emr/reference/plot_errorICD-1.png)
```r
#> $ICD
...
plot_errorICD(error$Error, ICDVersion = 9,
                           ...
                           Ranking = 3)
```
![pareto plot of error ICD9 (grouped)]
```r
$ICD
#>    ICDGroup groupCount CumCountPerc MostICDInGroup ICDPercInGroup IcdVersionInFile Suggestion
#> 1:        A         13        41.94         A01.05          61.54            ICD 9           
#> 2:        7          9        70.97          75.52          44.44            ICD 9           
#> 3:        0          5        87.10            001         100.00            ICD 9       0019
#> 4:   Others          4       100.00          E03.0         100.00            ICD 9           
```
