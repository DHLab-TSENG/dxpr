...
供後續 ICD 格式轉換的功能使用 (`convertIcdDecimaltoShort 和 convertIcdShortToDecimal`) 

ICD-9
```
icd9 <- ccsDxICD9$ICD
ICD_9_CM_withTwoFormat <- ICD_9_CM_GenerateDecimalFormat(icd9)
...
ICD_10_CM_withTwoFormat <- ICD_10_CM_GenerateDecimalFormat(icd10)
...
####單一階層分類 (CCS Category)
`groupIcdToCCS` 可取得 ICD-9-CM 及 ICD-10-CM 相對應的 CCS 分類及分類之敘述
...
`groupIcdToCCSLvl` 可取得 ICD-9-CM 及 ICD-10-CM 相對應的 CCS 多階層分類及分類之敘述
...
目前提供兩種方法:
 ...
 1) `groupIcdToCustomGrep`: 使用`窮舉法`的方式找出符合的ICD
```{r}
# groupingTable 
...
```
 2) `groupIcdToCustomGrepList`: 選擇`ICD開頭`有符合做為篩選的條件
```{r}
# grepTable
...
```
欲將 ICD 依照 AHRQ 共病症分類, 在 function `groupIcdToComorbid` 的 `comorbidMethod` 輸入 `AHRQ`
...
```
欲將 ICD 依照 Charlson 共病症分類, 在 function `groupIcdToComorbid` 的`comorbidMethod` 輸入 `Charlson`
...
```
欲將 ICD 依照 Charlson 共病症分類, 在 function `groupIcdToComorbid` 的`comorbidMethod` 輸入 `elix`
```
