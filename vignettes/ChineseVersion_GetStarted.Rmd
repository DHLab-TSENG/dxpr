...
vignette: >
  %\VignetteIndexEntry{Chinese Version_GetStarted}
...
```
### 分別產生 ICD-9-CM and ICD-10-CM 的兩種格式的表格: Decimal 和 Short (供後續 ICD 格式轉換的功能使用)
ICD-9
```
icd9 <- ccsDxICD9$ICD
...
```
當 CCS 分類結果為 `NA` , 會出現 `warning Message` , 可能是 ICD code 並未符合格式, 或是有其他的原因需要進行檢查確認
以範例為例, `42577` 為錯誤的 ICD-9-CM 之格式才會出現 `NA`
...
2) 篩選level 3: `42577` 為錯誤的ICD-9-CM之格式才會出現 `NA` , ICD-10-CM 的分類為 `NA`
...
如下面範例, 使用者想要找出符合高血壓及低血壓分類的 ICD code , 給予其對應的表格, 則會回傳有符合的分組, 未符合則回傳 `NA`
```
