...
#'
groupIcdDxToPhecode <- function(DxDataFile, idColName, icdColName, dateColName, icd10usingDate, isPhecodeDescription = TRUE){
  DxDataFile <- DxDataFile[ ,c(deparse(substitute(idColName)), deparse(substitute(icdColName)), deparse(substitute(dateColName)))]
  names(DxDataFile) <- c("ID", "ICD", "Date")
  Format <- ifelse(any(grepl("[.]", DxDataFile$ICD)), "Decimal", "Short")
  DxDataFile$ICDD <- convertIcdDxShortToDecimal(DxDataFile$ICD)$Decimal
  ...
  WrongFormat <- convertIcdDxShortToDecimal(DxDataFile$ICD)$Error
  ...
  if(anyNA(IcdToPhecode)){
    ...
    if(sum(is.na(IcdToPhecode)) > length(WrongFormat)){
      if(Format == "Short"){
        message(paste0("warning ICD: ", convertIcdDxDecimaltoShort(error_ICD$ICD)$Short, sep = "\t\n"))
      }else{
