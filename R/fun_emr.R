...
#' @param icd10usingDate the date that ICD-10 was started to be used in dxDataFile dataset. The data format should be YYYY/MM/DD or YYYY-MM-DD.
#' @param isDescription Category name or category description of standard classification methods for ICD diagnostic codes. By default, it is set to \code{True} (standard category description).
#' @param groupDataType  Four Stratified methods can be chosen: CCS (\code{'ccs'}), multiple-level CCS (\code{'ccslvl1'}, \code{'ccslvl2'}, \code{'ccslvl3'}, \code{'ccslvl4'}), PheWAS (\code{'PheWAS'}), comorbidities (\code{'ahrq'},\code{'charlson'}, \code{'elix'}), precise or fuzzy customized  method (\code{'customGrepIcdGroup'}, \code{'customIcdGroup'}). Default value is \code{"ccs"}.
...
#' @name dxCCS
#' @inherit common_DxArg
#' @param CCSLevel Used for multi-level CCS. By default, it is set to \code{1}. There is 4 multi-level CCS (1~4) for ICD-9, and 2 multi-level CCS (1 and 2) for ICD-10.
...
#' @inherit common_DxArg
#' @param dxDataFile a \code{groupedDT} format data frame generated from four strategies of code classification.
...
#' @name plotDiagCat
#' @param groupedDataWide Wide table of data frame (generated from \code{groupedDataLongToWide} function).
#' @param topN Numeric. Default is 10 (Top 10; 10 most common wrong ICD).
#' @param limitFreq Numeric. minimum frequency shown (frequency below this threshold will not be shown in plot). Default is 0.01. In other words, the threshold is 1% patient among total patients been diagnosed in the same diagnostic category.
